...
MAPPING = {
    "fenix": {"model": "api.FenixSampleCounts","schema_name":"org_mozilla_fenix", "apps": ["nightly", "beta", "release"]},
    "fog": {"model": "api.FOGSampleCounts", "schema_name":"firefox_desktop","apps": ["nightly", "beta", "release"]}
}
    ...
    def handle(self, bucket, *args, **options):
        ...
        for product, opts in MAPPING.items():
            ...
            for app_id in opts["apps"]:
                filename = f"glam-extract-{schema}_glam_{app_id}-sample-counts.csv"
                ...
                with connection.cursor() as cursor:
                    ...
                    cursor.execute(
                        f"CREATE TABLE {tmp_table} (LIKE glam_{product}_sample_counts)"
                    )
                    cursor.execute(

  ...
  const handleKeypress = (event) => {
    ...
    if (
      ...
      if (key === 'Enter') {
        ...
        focusedItem = 0;
        page.show(
          `/firefox/probe/${$searchResults.results[focusedItem].name}/explore?${$currentQuery}`
        );
      }
      if (key === 'Escape') {
...
<Portal>
  {#if $store.searchIsActive && $store.searchQuery.length}
    <div
      ...
      style="left: calc({x}px + var(--space-base)); width: calc({width}px -
      var(--space-base));"
      ...
      <div class="header-container">
        {#if $searchResults.total}
          <div class="header header--loaded" in:fly={{ x: -5, duration: 200 }}>
            <div>
              found {$searchResults.results.length} of {formatTotal($searchResults.total)}
              probes
            </div>
          </div>
        {:else}
        ...
        <ul
          ...
          {#each $searchResults.results as { id, name, type, description, versions }, i (id)}
            <li
              ...
              <div class="probe-type label label-text--01 label--{type}">
                {type}
              </div>

...
<g in:fade={fadeValues} class="{side}-axis">
  {#each TICKS as tick, i (tick)}
    {#if showTicks}
      <line
        class="tick"
        {...{ [`${mainDim}1`]: $bodyDimension + tickDirection * margins.buffer, [`${mainDim}2`]: tickEnd, [`${secondaryDim}1`]: mainScale(tick), [`${secondaryDim}2`]: mainScale(tick) }}
        stroke="var(--line-gray-01)"
        stroke-width="1" />
    {/if}
    {#if showBorder}
      <line
        {...{ [`${secondaryDim}1`]: mainScale.range()[0], [`${secondaryDim}2`]: mainScale.range()[1], [`${mainDim}1`]: $bodyDimension, [`${mainDim}2`]: $bodyDimension }}
        stroke="var(--line-gray-01)"
        stroke-width="1" />
    {/if}
    {#if showLabels}
      <text
        {...{ [`${mainDim}`]: $bodyDimension + tickDirection * margins.buffer + tickDirection * fontSizeCorrector, [`${secondaryDim}`]: mainScale(tick), dy: secondaryDim === 'y' ? '.35em' : undefined }}

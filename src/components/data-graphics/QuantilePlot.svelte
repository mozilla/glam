<script>
  ...
  export let width =
    getContext("width") ||
    DEFAULTS.elementWidth * data.length + margins.left + margins.right;
  ...
  function createYScale(values) {
    const scale = scalePoint()
      .domain(values)
      .range([margins.top + $bodyHeight, margins.top]);
    scale.type = "scalePoint";
    return scale;
  }
    ...
    return {
      ...
      onMousemove(e) {
        ...
        if (xScale.type === "scalePoint") {
          ...
          const xCandidates = xScale
            .domain()
            .filter(
              d =>
                xScale(d) - step / 2 < actualX &&
                xScale(d) < margins.left + $bodyWidth
            );
  ...
  let rollover;
  let onMousemove = e => {
    rollover.onMousemove(e);
  };
  let onMouseleave = e => {
    rollover.onMouseleave(e);
  };
  ...
  function initiateRollovers(parentSVG) {
    ...
    rolloverValues = derived(rollover, ({ x, y, py }) => {
      let hoverBin;
      let hoverHistogram;
      let hoverPoint;
      ...
      if (hoverKey) {
        ...
        hoverPercentiles = histogramValues.map(h => {
    ...
    <svg
      ...
      <g class="rollover-under">
        ...
        {#if $rolloverValues && $rolloverValues.hoverKey && aggregationLevel === 'build_id'}
          <Violin
            x={$rolloverValues.hoverKey}
            y={$rolloverValues.hoverHistogram}
            valueAccessor="bin"
        ...
        {#if aggregationLevel === 'build_id'}
          {#each histogramValues as { histogram, label }}
            <Violin
              ...
              densityRange={[0, 30]} />
          {/each}
          {#each PERCENTILES.map(p =>
            histogramValues.map(h => ({
              label: h.label,
        ...
        {#if aggregationLevel !== 'build_id'}
          <g in:fly={DEFAULTS.flyParams} class="bars">
            {#each histogramValues as { histogram, label }, i (label)}
              <Violin
                opacity={$rolloverValues !== undefined && label === $rolloverValues.hoverKey ? 1 : 0.6}
                x={label}
                y={histogram}
                valueAccessor="bin"
                densityAccessor="value" />
          ...
          <g class="box-plots">
            {#each histogramValues as { percentiles, label }, i (label)}
              <BoxPlot
      ...
      <g class="rollover-over">
        {#if $rolloverValues && $rolloverValues.hoverPoint}
          <path
            d={connectQuantiles($rolloverValues.hoverPercentiles)}
            stroke="var(--pantone-red-700)"
            opacity=".7"
            fill="none" />

          <circle
            cx={xScale($rolloverValues.x)}
            cy={yScale($rolloverValues.hoverPoint.value)}
            r="3"
            fill="blue" />

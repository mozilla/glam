<script>
  ...
  import { area, curveStepBefore as curve } from "d3-shape";

  export let orientation = "vertical";
  export let placementScale = getContext(
    orientation === "vertical" ? "xScale" : "yScale"
  );
  ...
  let valueScaleAdjustment =
    valueScaleType === "scalePoint" ? valueScale.step() / 2 : 0;
  ...
  let [histogramArea, inverseHistogramArea] = [1, -1].map(direction =>
    area()
  ...
  let histogramLine = histogramArea[derivedLine]()[binLocation](
    d => valueScale(d.bin) + valueScaleAdjustment
  );
  let inverseHistogramLine = inverseHistogramArea[derivedLine]()[binLocation](
    d => valueScale(d.bin) + valueScaleAdjustment
  );
  ...
  let translateY;
  $: translateX =
    orientation === "vertical" ? placementScale(placement) || rawPlacement : 0;
  $: translateY =
    orientation === "vertical" ? 0 : placementScale(placement) || rawPlacement;
...
{#if mounted}
  <g transform={translate} {opacity}>
    {#if showLeft}
      <path d={histogramArea(plotDensities)} fill={areaColor} {opacity} />
      <path

...
<div
  ...
  <Tweenable
    ...
    <ToplineRow
      ...
      {aggregationLevel}
      description="Set the reference point ⭑ by clicking on one of the graphs
      below.">
      <span slot="icon">⭑</span>
      <span slot="label">
    ...
    {#if dataLength > 1}
      <ToplineRow
        ...
        {aggregationLevel}
        description="Hover over the graphs below to compare the hover value ● to
        the reference ⭑; click to set the hover ● to the reference ⭑.">
        <span slot="icon">●</span>
        <span slot="label">
          ...
          {#if hovered}
            <div class="topline__client-count__comparison">
              ...
              <span style="font-weight: 500;">
                {formatParenPercent('.0%', absDiff(hovered.audienceSize, tweenValue, true), 7)}
              </span>
            </div>
          {/if}
        </span>

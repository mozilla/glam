<script>
  ...
  import { store, getFieldValueLabel, probe } from '../../state/store';
...
{#if $store.route.section === 'probe' && $probe}
  <div transition:fly={{ x: 5, duration: 200 }} class="main-filters">
    {#each Object.values(productConfig[$store.product].dimensions) as dimension, i (dimension.key)}
      {#if dimension.values.some((di) => dimension.isValidKey === undefined || dimension.isValidKey(di.key, $probe, store))}
        <DimensionMenu
          ...
          <div class="main-filter__label" slot="label">
            ...
            {getFieldValueLabel(dimension.key, $store.productDimensions[dimension.key])}
          ...
          <div slot="menu">
            <MenuList
              ...
              {#each dimension.values.filter((di) => dimension.isValidKey === undefined || dimension.isValidKey(di.key, $probe, store)) as { key, label }, i (key)}

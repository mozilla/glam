...
export function queryData(params, store) {
  const collection = `${params.channel}-${params.version}`;
  console.log(`Querying collection: ${collection} for metric: ${params.metric}`);
  db.collection(collection)
    ...
    .then((snapshot) => {
      snapshot.forEach((doc) => {
        store.storeData(doc.data().aggregates);
      });
    });
}

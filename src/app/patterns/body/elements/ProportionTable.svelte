<script>
  ...
  function getDomain() {
    return metricType === "proportions"
      ? [0, 1]
      : [Math.min, Math.max].map(f =>
          f(...data.map(d => f(...Object.values(d.counts))))
        );
  }
  ...
  const numberFormat =
    metricType === "proportions" ? formatPercent : formatCount;
</script>
      ...
      {#each activeBuckets as bucket, i}
        <th class="header-cell header-cell--text header-cell--percentile">
          <span
            ...
            style="background-color: {bucketColorMap(bucket)}" />
          {bucket}
        </th>
      {/each}
  ...
  <tbody>
    ...
    {#if totalPages > 1}
      <tr>
        ...
        <td class="header-cell footer-cell" colspan="100" />
      </tr>
    {/if}

...
export function twoPointSpring(
  ...
  const leftValues = spring(ptToSpringValue(initialHoverValue), {
    damping: 1,
    stiffness: 0.7
  });
  const rightValues = spring(ptToSpringValue(initialReferenceValue), {
    damping: 0.55,
    stiffness: 1
  });
...
export function cartesianCoordSpring(
  initialValue,
  xScale,
  yScale,
  params = { damping: 0.65, stiffness: 0.3 }
) {
  function ptToSpringValue(pt) {
    if (pt === undefined) return undefined;
    const out = { ...pt };

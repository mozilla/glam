...
<td class="data-cell data-cell--secondary">
  <div>
    <div class="audience-size">{countFormat($audienceSize)}</div>
    {#if mounted}
      <DataGraphic
        ...
        <line
          x1={audienceXScale(0)}
          x2={audienceXScale($audienceSize / total)}
          y1={3}
          y2={3}
          stroke-width="2"
        ...
        {#if isReference}
          <ReferenceSymbol
            xLocation={audienceXScale($audienceSize / total)}
            yLocation={3}
            color="var(--cool-gray-700)" />
        {:else}
          <circle
            cx={audienceXScale($audienceSize / total)}
            cy={3}
            r="2.5"
        ...
        {#if hovered}
          <ReferenceSymbol
            xLocation={audienceXScale(referenceCount / total)}
            yLocation={3}
            color="var(--cool-gray-500)" />
          <text
            font-size="10"
            font-weight="bold"
            text-anchor="middle"
            x={audienceXScale(referenceCount / total)}
            y={3 + 12}

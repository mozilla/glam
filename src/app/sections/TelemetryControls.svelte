...
<LeftDrawer {visible}>
  ...
  <div class="left-drawer__header">
    {#if !$hasDefaultControlFields}
      <div transition:fly={{ y: -10, duration: 200 }} href="#whatever">
        <Button on:click={resetFilters} level="medium" compact dark>
          reset filters
          <Cancel />
        </Button>
      </div>
  ...
  <Accordion bind:this={channel}>
    ...
    <span slot="description">
      {getFieldValueLabel('channel', $store.channel)}
    </span>
    <span slot="content">
      <RadioGroup
        onSelect={value => {
          store.setField('channel', value);
        }}>
        {#each CONFIG.fields.channel.values as { key, label }, i (key)}
          <RadioSelector value={key} {label} group={$store.channel} />
        {/each}
      </RadioGroup>
    </span>
  </Accordion>
  <Accordion bind:this={os}>
    ...
    <span slot="content">
      <RadioGroup
        onSelect={value => {
          store.setField('os', value);
        }}>
        {#each CONFIG.fields.os.values as { key, label }, i (key)}
          <RadioSelector value={key} {label} group={$store.os} />
        {/each}
      </RadioGroup>
    </span>
  </Accordion>
  <Accordion bind:this={aggregationLevel}>
    ...
    <span slot="description">
      {getFieldValueLabel('aggregationLevel', $store.aggregationLevel)}
    </span>
    <span slot="content">
      <RadioGroup
        onSelect={value => {
          store.setField('aggregationLevel', value);
        }}>
        {#each CONFIG.fields.aggregationLevel.values as { key, label }, i (key)}
          <RadioSelector value={key} {label} group={$store.aggregationLevel} />
        {/each}
      </RadioGroup>
    </span>
  </Accordion>

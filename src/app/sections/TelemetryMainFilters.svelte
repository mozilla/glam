<script>
  ...
  import { store, getFieldValueLabel } from "../state/store";
...
<div class="main-filters">
  <MenuButton
    ...
    <div slot="menu">
      <MenuList
        on:selection={event => {
          store.setField('channel', event.detail.key);
        }}>
  ...
  <MenuButton
    ...
    alignment="right">
    <div class="main-filter__label" slot="label">
      {getFieldValueLabel('os', $store.os)}
      <div class="pull-right-edge">
        <DownCarat size="14" />
      </div>
    </div>
    <div slot="menu">
      <MenuList
        on:selection={event => {
          store.setField('os', event.detail.key);
        }}>
  ...
  <MenuButton
    ...
    <div slot="menu">
      <MenuList
        on:selection={event => {
          store.setField('aggregationLevel', event.detail.key);
        }}>

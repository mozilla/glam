<script>
  ...
  const sslResumedSession = responseToData(
    SSL_RESUMED_SESSION.response,
    'proportion',
    'histogram-boolean'
  );
  const sslHandshakeVersion = responseToData(
    SSL_HANDSHAKE_VERSION.response,
    'proportion',
    'histogram-enumerated'
  );
  const cryptominersBlockedCount = responseToData(
    CRYPTO.response,
    'proportion',
    'histogram-categorical'
  );
  const gcReason2 = responseToData(
    GCREASON2.response,
    'proportion',
    'histogram-enumerated'
  );
...
<div class="story">
  ...
  {/each}
  <div
    style=" display:grid; grid-auto-flow: column; justify-content: start;
    grid-column-gap: var(--space-4x); margin-top: var(--space-2x);

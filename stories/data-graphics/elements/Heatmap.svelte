<script>
  ...
  import Heatmap from 'udgl/data-graphics/elements/Heatmap.svelte';
  ...
  import { byKeyAndAggregation } from '../../../src/utils/probe-utils';
  ...
  const median = gcms.map((d) => ({
    label: d.label,
    median: d.transformedPercentiles[50],
  }));
  let mounted = false;
  onMount(() => {
    mounted = true;

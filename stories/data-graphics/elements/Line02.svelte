          ...
          {#each getValue(data, value) as v, i}
            <Tweenable params={{ duration: 100 }} value={v} let:tweenValue>
              <text
                ...
                y={top + 12 * (i + 1)}
                font-size="12">
                <tspan>{Math.round(tweenValue)}</tspan>
                <tspan font-size="20" dx="2" dy="2" fill={cm[i]}>•</tspan>
              </text>
              <text
                x={right + 2}
                text-anchor="start"
        ...
        {#if value.x}
          <Springable value={get(data[0], value.x)} let:springValue>
            ...
            <text
              ...
              y={top - 8}>
              day {Math.round(springValue.currentIndex)}
            </text>
          </Springable>

          <Springable value={getXY(data, value)} let:springValue={spr}>
            {#each spr as { x, y }, i}
              <Point fill={cm[i]} {x} {y} r={3} />
            {/each}
          </Springable>
        {/if}
      </g>

...
<div
  style=" display: grid; grid-template-columns: auto max-content; width: 975px;
  align-items: center; margin-bottom: var(--space-4x); ">
...
<div class="multiples">
  {#each graphs as { name, type, key, yMax, axisFormat, hoverFormat }, i}
    <div>
      <div class="dg-header">
        <h2>{name}</h2>
        <div>{hoverFormat(hoverPt[key])}</div>
      </div>
      ...
      <DataGraphic
        ...
        <g slot="body">
          ...
          <Line
            lineDrawAnimation={{ duration: 1200 }}
            data={metricData}
            xAccessor="date"
            yAccessor={key} />
        </g>
  ...
  <DataGraphic
    ...
    <g slot="annotation" let:xScale let:yScale let:left let:right>
      ...
      <line
        ...
        y2={yScale(legendData[legendData.length - 1].yMax)}
        stroke="var(--cool-gray-500)" />
      <Point x={legendData[0].x} y={legendData[0].y} />
      <g
        style=" text-transform: uppercase; fill: var(--cool-gray-500);
        font-size: 10px; ">
        <g

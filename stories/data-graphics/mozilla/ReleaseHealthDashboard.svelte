...
{#if controlSet}
  <div
    style=" position: absolute; right:0; top:0; display: grid; grid-auto-flow:
    column; padding: var(--space-2x); background-color: white; height:100vh; ">
...
<div class="story">
  ...
  <div
    style="display: grid; grid-template-columns: auto auto; grid-column-gap:
    var(--space-2x); justify-content: start; grid-row-gap: var(--space-4x);">
        ...
        <DataGraphic
            ...
            <Axis side="bottom" ticks={xDomain}>
              ...
              <g slot="border">
                {#each Object.entries(byVersion) as [major, minors]}
                  <AxisLine
                    start={minors[0].minor}
                    end={minors.slice(-1)[0].minor} />
                {/each}
              </g>
              ...
              <g slot="labels" let:ticks>
                {#each Object.entries(byVersion) as [major, minors], i}
                  <AxisLabel
                    align={botAlign}
                    rotate={rot}
                    placement={minors[0].minor}
                    color="var(--cool-gray-450)">
                  ...
                  {#each minors as { minor, pass }, j}
                    ...
                    <AxisLabel
                      ...
                      placement={minor}
                      offset={24}
                      color={pass ? 'var(--cool-gray-400)' : 'var(--pantone-red-500)'}
                      fontSize={10}
          ...
          <g slot="body" let:xScale let:yScale let:top let:bottom>
            ...
            {#each releases as release, i}
              <g
                in:fly={{ duration: 500, delay: Math.random() * 300, y: 10, easing }}>
                <Point
